...
function Passreset() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/api/reset", { email });
      ...
      <div className="mt-7 bg-white rounded-xl shadow-lg dark:bg-white dark:border-black border-2 border-black">
        <div className="p-4 sm:p-7">
          <div className="text-center">
            <h1 className="block text-2xl font-bold text-gray-800 dark:text-black">Forgot password?</h1>
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              Remember your password?{' '}
              <a className="text-black decoration-2 hover:underline font-medium" href="/login">
                Login here
              </a>
            </p>
          </div>
          ...
          <div className="mt-5">
            <form onSubmit={handleSubmit}>
              <div className="grid gap-y-4">
                <div>
                  <label htmlFor="email" className="block text-sm font-bold ml-1 mb-2 dark:text-black">
                    Email address
                  </label>
